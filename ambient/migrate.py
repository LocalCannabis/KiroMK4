#!/usr/bin/env python3
"""
ambient/migrate.py — Run Ambient Intelligence Layer PostgreSQL migrations.

Usage:
    python -m ambient.migrate                   # Apply all pending migrations
    python -m ambient.migrate --print           # Print SQL without running
...
logger = logging.getLogger("ambient.migrate")
...
def run_migrations(cfg: dict, print_only: bool = False) -> None:
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute("""
                CREATE TABLE IF NOT EXISTS _ambient_migrations (
            ...
            cur.execute("SELECT filename FROM _ambient_migrations ORDER BY filename")
            applied = {row[0] for row in cur.fetchall()}

        pending = [f for f in migration_files if f not in applied]
        if not pending:
            logger.info("All ambient migrations already applied.")
            return

        for fname in pending:
            sql = _load_migration_sql(fname)
            logger.info("Applying ambient migration: %s", fname)
            with conn.cursor() as cur:
                cur.execute(sql)
                cur.execute(
                    "INSERT INTO _ambient_migrations (filename) VALUES (%s)",
                    (fname,),
                )
            conn.commit()
            logger.info("Applied: %s", fname)

        logger.info("All %d ambient migration(s) applied.", len(pending))
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Ambient Intelligence Layer migrations")
